...
echo "   Docker: docker-compose run --rm dodhooks-builder \\"
echo "            bash -c \"mkdir -p build && cd build && \\"
echo "            python3 ../configure.py --sm-path /sourcemod \\"
echo "            --mms-path /mmsource --target x86_64 --enable-optimize && \\"
echo "            ambuild\""
echo ""
